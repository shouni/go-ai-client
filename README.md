...
* **💡 インテリジェント・ペイロード管理:** * **File API 自動昇格:** `InlineData`（Base64）のサイズが閾値を超えた場合、ライブラリが内部で自動的に **Gemini File API** へアップロード。URI参照形式に置換して送信することで、巨大なペイロードによるAPIエラーを回避します。
* **スマート圧縮:** 送信前に画像をJPEG形式へ自動圧縮。通信効率を最大化し、AIモデルのメモリ消費を最適化します。
